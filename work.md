...
для использования регулярного выражения в переменной можно использовать запись между двумя //
``const emailRegExp = /^\S+@\S+\.\S+$/g``
проверка: `` emailRegExp.test(data) ``  // при совпадении вернется true 

для валидатора нужно дополнительно проверять - есть ли уже ошибка. если есть то ее не нужно дополнительно проверять на остальные ошибки....

## отправка данных формы

пока данные формы не будут валидные то кнопка должна быть неактивна - устанавливаем параметр у кнопки ``... disabled = {параметр_когда_действительные_данные }`` 
причем метод  validate() использовать нельзя так как мы войдем в бесконечный цикл...

## стили формы  - показать Пароль

для отображения пароля надо добавить в форму кнопку которая будет менять тип поля с password на text

так как открываем и закрываем пароль внутри формы и данные отобажения  не имеют никакого отношения к самой форме  и не меняют ее содержания то и информацию о том что пароль закрыт или открыт будем хранить в самом компоненте textField

добавляем кнопку в сам инпут и с её помощью будем менять тип поля

создаем новое состояние для пароля -открыт или закрыт через useState()

``const [showPassword, setShowPassword] = useState(false)``
далее создаем метод который меняет состояние отобажения пароля

``const toggleShowPassword = () => {setShowPassword((prevState) => !prevState)}``

изменяем тип поля по нажатию кнопки в поле ввода  ``onClick={ toggleShowPassword }``


кнопку в самом поле вода  отображаем если тип поля input type='password'
...
изменяем тип поля: __по нажатию кнопки в поле ввода__
изменяем состояние __showPassword__ через __setShowPassword__
 и от этого состояния меняем тип поля в поле ввода пароля
